...
foreach($acf_config['fields'] as $field_name => $value) {
 ...
 var check_acf_<?php echo $field_name; ?> = function(event, show_type, is_submit) {
        ...
        <?php
        // Basic Validation
        if( $value['validation']['basic']['value'] == 1 && $value['multiple'] != 1 && !is_array($value['type']['radios']) ) {
        ?>
        ...
        if( $value['validation']['min_selections']['value'] > 0 && is_array($value['type']['checkboxes']) ) {    
        ?>
        ...
        // Minimum Selections Validation (Checkboxes)
        if( is_array($value['type']['radios']) && $value['validation']['basic']['value'] == 1 ) {    
        ?>
            ...
            if(radios_length == 0) {
                ...
                var afterField = '<div '+ forSlideStyle +' id="acf_<?php echo $field_name; ?>_error" class="afb_error"><?php echo $value['validation']['radio']['message']; ?></div>';
